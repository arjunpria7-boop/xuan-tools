    ...
    const imageDisplay = (
      <div className="relative">
        ...
        <img
            ...
            className={`absolute top-0 left-0 w-full h-auto object-contain max-h-[60vh] rounded-xl transition-opacity duration-200 ease-in-out ${isComparing ? 'opacity-0' : 'opacity-100'} ${activeTab === 'retus' ? 'cursor-crosshair' : ''}`}
        />
      </div>
    );
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        <div className="relative w-full shadow-2xl rounded-xl overflow-hidden bg-black/20">
            ...
            {displayHotspot && !isLoading && activeTab === 'retus' && (
                <div 
                    className="absolute rounded-full w-6 h-6 bg-pink-500/50 border-2 border-white pointer-events-none -translate-x-1/2 -translate-y-1/2 z-10"
                    style={{ left: `${displayHotspot.x}px`, top: `${displayHotspot.y}px` }}
                >
                    <div className="absolute inset-0 rounded-full w-6 h-6 animate-ping bg-pink-400"></div>
                </div>
            )}
        </div>
        ...
        <div className="w-full bg-gray-800/80 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-sm">
            {(['retus', 'potong', 'sesuaikan', 'filter'] as Tab[]).map(tab => (
                 <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`w-full capitalize font-semibold py-3 px-5 rounded-md transition-all duration-200 text-base ${
                        activeTab === tab 
                        ? 'bg-gradient-to-br from-red-500 to-pink-500 text-white shadow-lg shadow-pink-500/40' 
                        : 'text-gray-300 hover:text-white hover:bg-white/10'
                    }`}
                >
                    {tab === 'potong' ? 'Potong' : tab === 'sesuaikan' ? 'Sesuaikan' : tab === 'filter' ? 'Filter' : 'Retus'}
                </button>
            ))}
        </div>
        ...
        <div className="w-full">
            {activeTab === 'retus' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-gray-400">
                        {editHotspot ? 'Bagus! Sekarang deskripsikan editan Anda di bawah.' : 'Klik sebuah area pada gambar untuk melakukan editan yang presisi.'}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex flex-col items-center gap-2">
                        <div className="w-full flex items-center gap-2">
                            <input
                                ...
                                placeholder={editHotspot ? "contoh: 'ubah warna baju saya menjadi biru'" : "Pertama, klik sebuah titik pada gambar"}
                                className="flex-grow bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-5 text-lg focus:ring-2 focus:ring-pink-500 focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60"
                                disabled={isLoading || !editHotspot}
                            />
                            <button 
                                type="submit"
                                className="bg-gradient-to-br from-red-600 to-pink-500 text-white font-bold py-5 px-8 text-lg rounded-lg transition-all duration-300 ease-in-out shadow-lg shadow-red-500/20 hover:shadow-xl hover:shadow-red-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner disabled:from-red-800 disabled:to-pink-700 disabled:shadow-none disabled:cursor-not-allowed disabled:transform-none"
                        ...
                        {editHotspot && (
                            <div className="flex flex-wrap justify-center gap-2 mt-3 animate-fade-in">
                                <p className="w-full text-center text-sm text-gray-500 mb-1">Coba contoh ini:</p>
                                {['hapus objek ini', 'ganti warna menjadi biru', 'buat ini bersinar'].map(p => (
                                    <button
                                        type="button"
                                        key={p}
                                        onClick={() => setPrompt(p)}
                                        className="bg-white/5 border border-white/10 text-gray-300 text-sm font-medium py-1.5 px-3 rounded-full hover:bg-white/10 transition-colors"
                                    >
                                        "{p}"
                                    </button>
                                ))}
            ...
            {activeTab === 'potong' && <CropPanel onApplyCrop={handleApplyCrop} onSetAspect={setAspect} isLoading={isLoading} isCropping={!!completedCrop?.width && completedCrop.width > 0} />}
            {activeTab === 'sesuaikan' && <AdjustmentPanel onApplyAdjustment={handleApplyAdjustment} isLoading={isLoading} />}
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            <button 
                ...
                aria-label="Batalkan tindakan terakhir"
            >
            ...
            {canUndo && (
              <>
                ...
                <button
                  ...
                  className="flex items-center justify-center text-center bg-white/10 border border-white/20 text-gray-200 font-semibold py-3 px-5 rounded-md transition-all duration-200 ease-in-out hover:bg-white/20 hover:border-white/30 active:scale-95 text-base"
                  aria-label="Buka pratinjau gambar yang bisa diperbesar"
                >
            ...
            <button 
                ...
                className="flex-grow sm:flex-grow-0 ml-auto bg-gradient-to-br from-blue-500 to-purple-600 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-blue-500/20 hover:shadow-xl hover:shadow-blue-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base"
            >
                Unduh Gambar
            </button>
        </div>
